...
function Offline() {
  return (
    <section className="rounded-[4px] border border-slate-800 bg-slate-900/40 p-8">
      <div className="font-mono text-[11px] uppercase tracking-widest text-sky-400 flex items-center gap-3 mb-4">
        <span className="inline-block w-4 h-px bg-sky-400" />
        STEP 1 OF 4 · ONE-TIME SETUP
      </div>
      <h2 className="text-2xl font-bold text-slate-50 mb-3">Let&rsquo;s get your local bridge running.</h2>
      <p className="text-slate-300 mb-6 text-[15px] leading-relaxed">
        This dashboard talks to a tiny CLI you run on your own machine — that&rsquo;s how nothing has to leave your network. Three commands and you&rsquo;re live. Total time: about five minutes.
      </p>

      <div className="space-y-4">
        <SetupCard step="A" title="Install Rust (skip if you already have it)" note="One-time. Adds rustup + cargo to ~/.cargo/bin.">
          <code className="block font-mono text-[13px] text-sky-300 bg-slate-950 border border-slate-800 px-4 py-3 rounded-[4px] overflow-x-auto whitespace-pre">curl --proto &apos;=https&apos; --tlsv1.2 -sSf https://sh.rustup.rs | sh</code>
        </SetupCard>
        <SetupCard step="B" title="Install learn-rv from source" note="Compiles in 3-5 min. Installs a `learn` binary you can run from anywhere.">
          <code className="block font-mono text-[13px] text-sky-300 bg-slate-950 border border-slate-800 px-4 py-3 rounded-[4px] overflow-x-auto whitespace-pre">cargo install --git https://github.com/stuinfla/learner-rv learn-cli</code>
        </SetupCard>
        <SetupCard step="C" title="Start the bridge" note="Leave this terminal running. This page will auto-detect it within 5 seconds.">
          <code className="block font-mono text-[13px] text-sky-300 bg-slate-950 border border-slate-800 px-4 py-3 rounded-[4px] overflow-x-auto whitespace-pre">learn ui</code>
        </SetupCard>
      </div>

      <div className="mt-6 flex items-center gap-2 text-slate-500 text-xs">
        <span className="inline-block w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" />
        <span>polling <code className="font-mono text-slate-400">127.0.0.1:7878</code> every 5 seconds…</span>
      </div>

      <p className="mt-6 text-slate-500 text-xs">
        Need product context first? <a href="/" className="text-sky-400 hover:text-sky-300 underline decoration-sky-500/40 underline-offset-2">Read the homepage</a>.
      </p>
    </section>
  );
}

function SetupCard({ step, title, note, children }: { step: string; title: string; note: string; children: React.ReactNode }) {
  return (
    <div className="border border-slate-800 bg-slate-950/60 rounded-[4px] p-5">
      <div className="flex items-baseline gap-3 mb-3">
        <span className="font-mono text-[11px] text-slate-500 border border-slate-700 px-1.5 py-0.5 rounded-[2px]">{step}</span>
        <h3 className="text-[15px] font-semibold text-slate-100">{title}</h3>
      </div>
      {children}
      <p className="mt-3 text-slate-500 text-xs">{note}</p>
    </div>
  );
}
